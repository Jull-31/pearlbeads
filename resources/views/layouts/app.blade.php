...
<body class="bg-gray-50">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2 md:space-x-4">
                    <button onclick="cart.toggleCartSidebar()" class="relative p-2 text-gray-800 hover:text-purple-600 transition">
                        ...
                        <span id="cartBadge" class="absolute top-1 right-1 bg-red-500 text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full scale-0 transition-transform border border-white">0</span>
                    </button>
    ...
    <div id="cartOverlay" onclick="cart.toggleCartSidebar()" class="hidden fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity"></div>
    ...
    <div id="cartSidebar" class="fixed top-0 right-0 h-full w-[85%] md:w-96 bg-gray-50 shadow-2xl transform translate-x-full transition-transform duration-300 z-50">
        <div class="flex flex-col h-full">
            <div class="bg-purple-600 text-white p-4 flex justify-between items-center">
                ...
                <button onclick="cart.toggleCartSidebar()" class="p-2 hover:bg-purple-700 rounded transition">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="border-t bg-white p-4 space-y-4">
                <div class="flex justify-between items-center font-bold">
                    <span>Total:</span>
                    <span id="cartTotal" class="text-purple-600">Rp 0</span>
                </div>
                <button onclick="cart.openOrderModal()" class="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-lg font-bold flex items-center justify-center space-x-2 transition shadow-lg text-sm">
    ...
    <div id="orderModal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" onclick="cart.closeOrderModal()"></div>
        ...
        <div class="bg-white w-full max-w-sm rounded-2xl shadow-2xl relative z-10 overflow-hidden transform transition-all scale-100">
            <div class="bg-purple-600 p-4 text-center text-white relative">
                <button onclick="cart.closeOrderModal()" class="absolute top-4 right-4 text-white/80 hover:text-white font-bold text-xl">✕</button>
                <h3 class="font-bold text-xl">Lengkapi Pesanan 📝</h3>
            </div>
            
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Nama Kamu</label>
                    <input type="text" id="customerName" placeholder="Contoh: pearlbeads" 
                           class="w-full border-2 border-gray-200 rounded-lg p-3 focus:border-purple-500 outline-none">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Jadwal Ambil</label>
                    <input type="datetime-local" id="pickupTime" 
                           class="w-full border-2 border-gray-200 rounded-lg p-3 focus:border-purple-500 outline-none text-gray-600">
                </div>
                
                <div class="flex gap-2 pt-2">
                    <button onclick="cart.closeOrderModal()" class="flex-1 bg-gray-100 text-gray-700 font-bold py-3 rounded-lg hover:bg-gray-200 transition">Batal</button>
                    <button onclick="cart.confirmAndSendWA()" class="flex-1 bg-green-500 text-white font-bold py-3 rounded-lg hover:bg-green-600 shadow-lg transition">
                        Kirim WA ✈️
                    </button>
                </div>
            </div>
